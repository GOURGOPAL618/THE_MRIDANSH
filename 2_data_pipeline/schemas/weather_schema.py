...
class WeatherTelemetry(BaseModel):
    ...
    precipitation_mm: float = Field(
        ..., ge=0.0, le=1000.0, description="Precipitation in mm/day"
    )
    surface_temperature_k: float = Field(
        ..., ge=200.0, le=350.0, description="Temperature in Kelvin"
    )
...
if __name__ == "__main__":
    sample_weather = WeatherTelemetry(
        precipitation_mm=14.5, surface_temperature_k=301.15, relative_humidity=65.0
    )

    print("✅ Weather Schema Validation Passed!")
    print(
        f"Precipitation: {sample_weather.precipitation_mm}mm | Temp: {sample_weather.surface_temperature_k}K"
    )
