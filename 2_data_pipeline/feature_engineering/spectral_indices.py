...
logging.basicConfig(
    level=logging.INFO, format="%(astime)s - %(lavelname)s - %(message)s"
)
...
class VegetationIndicesEngine:
    ...
    def calculate_evi(
        self, blue: np.ndarray, nir: np.ndarray, red: np.ndarray
    ) -> np.ndarray:
    ...
    def calculate_savi(
        self, red: np.ndarray, nir: np.ndarray, l_factor: float = 0.5
    ) -> np.ndarray:
    ...
    def compute_all_indices(
        self, optical_bands: dict[str, np.ndarray]
    ) -> dict[str, np.ndarray]:
