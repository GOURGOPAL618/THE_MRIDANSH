...
from ..schemas.satellite_schema import SentinelFrameMetadata, SpatialBoundingBox
...
class SentinelDataIngestor:
    ...
    def fetch_optical_frames(
        self,
        bbox: SpatialBoundingBox,
        start_date: str,
        end_date: str,
        max_cloud_cover: float = 20.0,
    ) -> list[SentinelFrameMetadata]:
        """Simulates/Triggers Sentinel-2 Optical Multi-Spectral Ingestion Pipeline"""
        logging.info(
            f"🛰️ Requesting Sentinel - 2 Optical Bands for BBox: {bbox.bbox} | Range: {start_date} to {end_date}"
        )
        ...
        logging.info(
            f"✅ Sentinel-2 Frame Successfully Ingested: {sample_frame.scene_id}"
        )
        return [sample_frame]

    def fetch_radar_sar_frames(
        self,
        bbox: SpatialBoundingBox,
        start_date: str,
        end_date: str,
        ...
        logging.info(
            f"📡 Requesting Sentinel-1 SAR Radar Arrays ({polarization}) for BBox: {bbox.bbox}"
        )
...
if __name__ == "__main__":
    ...
    optical_payload = ingestor.fetch_optical_frames(
        test_bbox, "2026-07-22", "2026-07-25"
    )
    radar_payload = ingestor.fetch_radar_sar_frames(
        test_bbox, "2026-07-22", "2026-07-25"
    )
