...
class RasterDataProcessor:
    ...
    def generate_cloud_mask(
        self, scl_band: np.ndarray, cloud_values: list = [3, 8, 9, 10]
    ) -> np.ndarray:
        """Generates a binary cloud/shadow mask based on Sentinel-2 Scene Classification (SCL)."""
        logging.info(
            "☁️ Filtering Cloud Noise & Shadow Artifacts via SCL Band Masking..."
        )
        ...
        logging.info(
            f"✅ Cloud Mask Created: {cloud_percentage:.2f}% pixels flagged as unusable cloud/shadow."
        )
        return cloud_mask

    def apply_vector_polygon_clip(
        self, raster_data: np.ndarray, polygon_mask: np.ndarray
    ) -> np.ndarray:
        """Clips and crops full satellite raster matrix strictly inside the field boundary polygon."""
        logging.info(
            "✂️ Clipping Satellite Raster to Field Vector Polygon Boundaries..."
        )
    ...
    def resample_raster_grid(
        self, raster_matrix: np.ndarray, target_shape: tuple[int, int]
    ) -> np.ndarray:
        """Resamples raw multi-spectral raster grid to target uniform resolution grid shape."""
        logging.info(
            f"🔄 Resampling Raster Matrix from {raster_matrix.shape} -> Target shape: {target_shape}"
        )

        # Nearest-neighbor grid indexing simulation for spatial grid alignment
        row_indices = np.linspace(
            0, raster_matrix.shape[0] - 1, target_shape[0]
        ).astype(int)
        col_indices = np.linspace(
            0, raster_matrix.shape[1] - 1, target_shape[1]
        ).astype(int)
...
if __name__ == "__main__":
    ...
    mock_scl = np.random.choice(
        [4, 4, 4, 3, 8, 9], size=(100, 100)
    )  # 4=Vegetation, 3/8/9=Cloud
    ...
    resampled_band = processor.resample_raster_grid(
        clipped_band, target_shape=(120, 120)
    )
