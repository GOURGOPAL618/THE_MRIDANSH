...
class COGRasterProcessor:
    ...
    def window_read_synthetic_raster(
        ...
        mask = np.random.choice([True, False], size=target_shape, p=[0.05, 0.95])
    ...
    def align_and_resample(
        ...
        row_indices = np.linspace(0, raster_data.shape[0] - 1, target_shape[0]).astype(
            int
        )

        col_indices = np.linspace(0, raster_data.shape[1] - 1, target_shape[1]).astype(
            int
        )
    ...
    def clean_and_normalize_raster(
        ...
        metadata = {
            ...
            "nan_percentage": float(np.round((nan_count / data.size) * 100, 2)),
...
if __name__ == "__main__":
    ...
    print(
        f"   NaN Pixels Filled: {meta['nan_filled_count']} ({meta['nan_percentage']}%)"
    )
