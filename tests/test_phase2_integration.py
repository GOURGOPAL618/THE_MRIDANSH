...
def test_phase2_pipeline():
    ...
    print(f"  ✓ Health Telemetry Status: ONLINE | Service: {health_json.get('service')}")
    ...
    print(f"  ✓ V1 Subsystem: ONLINE | Active Routes: {len(v1_json.get('active_endpoints', []))}")
    ...
    assert res_predict.status_code == 200, f"Predict route failed: {res_predict.status_code}"
    pred_json = res_predict.json()
    assert pred_json.get("status") == "SUCCESS"
    assert len(pred_json.get("predicted_moisture_m3m3", [])) == 5
    print(f"  ✓ Richards-EnKF Physics Output Validated | Execution Time: {pred_json.get('execution_time_sec')}s")
    ...
    print(f"  ✓ GIS Sub-Grid Mesh GeoJSON Validated | Received Polygons: {features_len}")
