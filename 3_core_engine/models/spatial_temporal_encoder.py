...
class SpatialTemporalEncoderBackbone:
    ...
    def __init__(
        self, in_channels: int = 8, feature_dim: int = 128, sequence_length: int = 5
    ):
        self.in_channels = in_channels
        self.feature_dim = feature_dim
        self.sequence_length = sequence_length
        logging.info(
            f"🧠 Initializing Neural Backbone | Channels: {in_channels} | Latent Dim: {feature_dim}"
        )
    ...
    def extract_temporal_sequence(self, sequence_tensor: np.ndarray) -> np.ndarray:
        ...
        logging.info(
            f"⏳ Processing Temporal Sequence Stream: {sequence_tensor.shape} (Batch x Time x H x W x C)..."
        )
        ...
        for t in range(time_steps):
            frame_spatial = self.extract_spatial_features(
                sequence_tensor[:, t, :, :, :]
            )
...
if __name__ == "__main__":
    ...
    mock_sequence = np.random.uniform(0.0, 1.0, size=(2, 5, 120, 120, 8)).astype(
        np.float32
    )

    backbone = SpatialTemporalEncoderBackbone(
        in_channels=8, feature_dim=128, sequence_length=5
    )
