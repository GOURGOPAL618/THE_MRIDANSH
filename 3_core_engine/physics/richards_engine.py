...
class RichardsSoilPhysicsEngine:
    ...
    def __init__(self, alpha: float = 0.035, n_param: float = 1.48, ks_sat: float = 10.5):
    ...
    def calculate_relative_hydraulic_conductivity(self, volumetric_water_content: np.ndarray, theta_res: float = 0.078, theta_sat: float = 0.43) -> np.ndarray:
    ...
    def compute_richards_flux(self, moisture_grid: np.ndarray, depth_interval_m: float = 0.1) -> Dict[str, np.ndarray]:
        """Calculates 1D Richards equation hydraulic head gradient & water flus divergence."""
        logging.info("📐 Executing 1D Unsaturated Soil Richards Equation Flux Computation...")
        ...
        logging.info(f"✅ Physics Engine Processed; Mean Water Flux: {np.mean(water_flux):.4f} cm/day | Loss Penalty = {physics_loss:.6f}")
...
if __name__ == "__main__":
    ...
    physics_results = physics_engine.compute_richards_flux(mock_moisture, depth_interval_m = 0.1)
