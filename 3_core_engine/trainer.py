...
class MultiTaskTrainingOrchestrator:
    ...
    def __init__(
        ...
        self.encoder = SpatialTemporalEncoderBackbone(in_channels = in_channels, feature_dim = feature_dim)
    ...
    def train_epoch(self, datacube_batch: np.ndarray, target_agro: np.ndarray, target_civil: np.ndarray) -> Dict[str, float]:
        ...
        latent_map = latent_output["latent_representation"]  #Shape: (Batch, H, W, Latent_Dim)

        # 2. Multi-Task Simulated Predictions (Agro Soil Moisture & Civil Bearing Capacity)
        pred_agro = np.mean(latent_map, axis = -1, keepdims = True) * 0.35     # Scaled Volumetric Soil Moisture
        pred_civil = np.mean(latent_map, axis = -1, keepdims = True) * 150.0   # Scaled Bearing Capacity (kPa)
        ...
        loss_dict = self.compute_composite_loss(
            pred_agro_sq, target_agro,
            pred_civil_sq, target_civil,
            physics_penalty
        )
...
if __name__ == "__main__":
    ...
    mock_datacube = np.random.uniform(0.0, 1.0, size = (2, 5, 120, 120, 8)).astype(np.float32)

    # Mock Target Ground Truth Matrices
    mock_target_agro = np.random.uniform(0.15, 0.40, size = (2, 120, 120)).astype(np.float32)
    mock_target_civil = np.random.uniform(50.0, 200.0, size = (2, 120, 120)).astype(np.float32)
